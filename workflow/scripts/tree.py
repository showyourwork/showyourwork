...
tex_files = snakemake.config["tex_files"]
script_extensions = snakemake.config["script_extensions"]
...
# Load the XML tree
root = ParseXMLTree(paths.temp / "showyourwork.xml").getroot()
...
for figure in root.findall("FIGURE"):
    ...
    if len(labels) and labels[0].text is not None:
        ...
        for ext in script_extensions:
            ...
            script = (paths.figure_scripts / f"{label}.py").as_posix()
    ...
    graphics = [
        (paths.tex / graphicspath / graphic.text).resolve().as_posix()
...
with open(paths.temp / "tree.json", "w") as f:
