...
class BaseClass(TemporaryShowyourworkRepository):
    ...
    def build_local(self):
        ...
        get_stdout("CI=false showyourwork build --conda-frontend micromamba", shell=True, cwd=self.cwd)

        # Delete all output, but keep the cache
        if not self.cache:
            get_stdout("mv .showyourwork/cache .", shell=True, cwd=self.cwd)
        get_stdout("CI=false showyourwork clean --force", shell=True, cwd=self.cwd)
        if not self.cache:
            get_stdout(
