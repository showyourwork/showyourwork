...
\begin{description}[style=nextline]
    \item[How do I include figures that aren't generated by a \texttt{Python} script?]
    ...
    %
    \item[How do I specify custom workflow dependencies?]
    The \texttt{showyourwork} workflow uses \texttt{Snakemake} to build a simple dependency graph for the article: the article PDF depends on the figure files, which in turn depend on the figure scripts. Sometimes it's useful to specify additional dependencies, such as when a script imports a function from a different script, or when a script depends on a dataset that can potentially change. In that case, the user must tell \texttt{Snakemake} about this dependency so that it knows to re-run the figure script (and re-compile the paper) when, e.g., a dataset changes. This feature is under development and will be implemented soon!
    %
    \item[What if I don't use \verb+\includegraphics+ to include my figures?]
