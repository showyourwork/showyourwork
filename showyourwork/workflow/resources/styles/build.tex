  ...
  \AtBeginDocument{%
    ...
    \href{\syw@url/tree/\syw@sha/}{%
      \begin{tikzpicture}[remember picture,overlay]%
        \node (STAMP) at ($(current page.north east) - (\syw@stampX in + \syw@stampXOffset in + \syw@stampRadius in, \syw@stampY in + \syw@stampYOffset in + \syw@stampRadius in)$) [rotate=\syw@stampAngle] {\includegraphics[width=\syw@stampSize in]{showyourwork-stamp.pdf}};%
        %
        \draw[draw=none,postaction={decorate},decoration={text along path,text={|\syw@stampFormat|\syw@slug},text align=center,text color=sywStampRed}]
        ($(STAMP.center) - (\syw@stampOuterRadius in, 0 in)$) arc [start angle=180,end angle=360,radius=\syw@stampOuterRadius in];
      \end{tikzpicture}%
    }%
    \makeatother%
  }
